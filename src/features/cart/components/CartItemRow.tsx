...
export default function CartItemRow({
 ...
 {/* Image */}
 <div className="w-14 h-14 rounded-xl bg-custom-tertiary overflow-hidden shrink-0">
 {item.image ? (
 <img
 src={item.image}
 alt={item.name}
 ...
 <div className="font-medium text-custom-primary leading-5 truncate">
 {item.name}
 </div>
 {item.description ? (
 ...
 type="button"
 onClick={dec}
 className="w-8 h-8 rounded-full border border-custom-primary bg-custom-card flex items-center justify-center hover:bg-custom-light"
 aria-label="Decrease quantity"
 >
 <HiMinus className="w-4 h-4 text-custom-secondary"/>
 </button>

 <div className="w-6 text-center text-sm font-medium text-custom-primary">
 {item.quantity}
 </div>
