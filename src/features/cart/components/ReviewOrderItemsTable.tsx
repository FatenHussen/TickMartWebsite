...
type ReviewOrderItemsTableProps = {
  items: CartItem[];
};

export default function ReviewOrderItemsTable({
  items,
}: ReviewOrderItemsTableProps) {
    ...
    <div className="bg-white rounded-xl border border-custom-secondary shadow-sm overflow-hidden">
      <div className="overflow-x-auto rounded-xl">
        <table className="w-full">
          ...
          <tbody>
            {items.map((item) => (
              <ReviewOrderItemRow key={item.id} item={item} isRTL={isRTL} />
            ))}
          </tbody>
        </table>
      </div>
...
type ReviewOrderItemRowProps = {
  item: CartItem;
  isRTL: boolean;
};

function ReviewOrderItemRow({ item, isRTL }: ReviewOrderItemRowProps) {
  return (
