...
export function inferFavoriteType(item: FavoriteItem): FavoriteType {
  ...
  const itemAny = item as { next_delivery_date?: string; items_count?: number };
  if ("title" in item && (itemAny.next_delivery_date != null || (itemAny.items_count ?? 0) > 1))
    return "basket";
  return "product";
}
...
export function normalizeFavoriteItem(
  ...
  const topBadgesRaw = item.top_badges ?? item.budges ?? [];
  const topBadges: { id: number; name: string; color: string; image?: string }[] =
    topBadgesRaw.map((b) => ({
      id: b.id,
      name: b.name,
      color: b.color,
      image: "image" in b ? (b as { image?: string }).image : undefined,
    }));
